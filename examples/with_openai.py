"""Full loop with OpenAI: fetch → judge with GPT → rank → display.

Mirror of ``with_claude.py`` using the OpenAI Python SDK instead of
Anthropic's. The three provider examples (``with_claude.py``,
``with_openai.py``, ``with_gemini.py``) are kept deliberately parallel
so you can diff them to see exactly where provider SDKs differ — and
pick whichever one matches your account.
...
MAX_SURFACED = 5
WINDOW_DAYS = 7
JUDGE_BATCH = 15  # candidates sent to OpenAI in one prompt
# Swap this to whatever model you have access to. gpt-4o is a reasonable
# default that accepts the `max_tokens` parameter used below. If you
# switch to an o1/o3 reasoning model, rename to `max_completion_tokens`.
OPENAI_MODEL = "gpt-4o"


def _require_openai() -> Any:
    """Import the OpenAI SDK with a friendly error on missing install."""
    try:
        import openai  # noqa: PLC0415 — lazy import keeps optional dep
    except ImportError:
        print(
            "This example needs the OpenAI SDK:\n    pip install openai\n",
        )
        sys.exit(1)
    return openai
...
def main() -> None:
    """Run the full GPT-judged digest loop."""
    ...
    with tempfile.TemporaryDirectory(prefix="arxiv-mcp-openai-") as tmp:
        ...
        # ── Step 2: Ask GPT to score the batch ─────────────────────────
        rubric = render_research_judge_rubric(
            rubric_focus=topic.rubric_focus,
            papers=papers_for_judge,
        )

        print(f"[2/3] Asking {OPENAI_MODEL} to score {len(batch)} papers...")
